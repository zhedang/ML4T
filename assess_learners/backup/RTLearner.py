    ...
    def build_tree(self, data):
        if data.shape[0] <= self.leaf_size:
            return np.atleast_2d(
                np.array(["leaf", np.mean(data[:, -1]), np.nan, np.nan], dtype=object))  # 2d fix the one row issue
        elif len(np.unique(data[:, -1])) == 1:
            return np.atleast_2d(np.array(["leaf", data[0, -1], np.nan, np.nan], dtype=object))
        else:
            index= np.random.randint(0,data.shape[1]-2)
...
if __name__ == "__main__":
    ...
    learner = RTLearner(leaf_size=1, verbose=False)  # Try different leaf_size values
